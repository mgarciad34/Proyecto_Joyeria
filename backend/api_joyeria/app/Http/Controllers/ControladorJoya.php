...
class ControladorJoya extends Controller
{
    ...
    function getRecomendaciones($parametro)
    {
        try {
            ...
            GROUP BY j.id HAVING COUNT(dr.id_componente) =
            (SELECT COUNT(*) FROM detalle_recetas WHERE id_joya = j.id)
            AND MIN(tc.cantidad / dr.cantidad)>=1 order by id_joya desc
        ');
            }
            if ($parametro == 3) {
                ...
                (SELECT COUNT(*) FROM detalle_recetas WHERE id_joya = j.id)
                AND MIN(tc.cantidad / dr.cantidad)>=1
                order BY veces_fabricacion DESC
        ');
            }
            if ($parametro == 2) {

                $joyas = DB::select('SELECT j.id AS id_joya, COUNT(h.id) AS veces_fabricada
                FROM joyas j
                LEFT JOIN detalle_recetas dr ON j.id = dr.id_joya
                LEFT JOIN tipos_componentes tc ON dr.id_componente = tc.id
                LEFT JOIN historico_joyas h ON j.id = h.id_joya
                GROUP BY j.id
                HAVING MIN(tc.cantidad / dr.cantidad) >= 1
                ORDER BY veces_fabricada');
            }
            if ($parametro == 1) {
                $joyas = DB::select('SELECT j.id AS id_joya, COUNT(h.id) AS veces_fabricada
                FROM joyas j 
